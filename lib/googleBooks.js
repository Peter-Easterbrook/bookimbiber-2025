...
export const searchBooks = async (query, maxResults = 10) => {
  try {
    ...
    // Remove language restriction to allow books in any language; order by relevance
    ...
    // Transform Google Books data to our app format (no language filtering)
    const transformedBooks = data.items.map((item) =>
      transformGoogleBookData(item)
    );

    // Return all transformed books without filtering by language
