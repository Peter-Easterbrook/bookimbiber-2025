 ...
 * @returns {Promise<Array>} Array of book objects in multiple languages (local + English)
 */
export const searchBooks = async (query, maxResults = 10) => {
  try {
    ...
    // Make multiple searches to ensure we get both local language and English results
    const searches = [
      // Primary search without language restriction
      `${formattedQuery}`,
      // Explicit English search to ensure English books are included
      `${formattedQuery} language:en`,
      // Also try with printType=books to get more comprehensive results
      `${formattedQuery} printType:books`
    ];

    const allBooks = new Map(); // Use Map to avoid duplicates by googleBooksId
    const resultsPerSearch = Math.ceil(maxResults / 2); // Split results between searches

    for (const searchQuery of searches) {
      try {
        const encodedQuery = encodeURIComponent(searchQuery);
        const url = `${GOOGLE_BOOKS_API}?q=${encodedQuery}&maxResults=${resultsPerSearch}&orderBy=relevance`;

        console.log('Searching Google Books API with query:', searchQuery);
        ...
        // Transform and add unique books
        data.items.forEach(item => {
          const transformedBook = transformGoogleBookData(item);
          if (transformedBook.googleBooksId && !allBooks.has(transformedBook.googleBooksId)) {
            allBooks.set(transformedBook.googleBooksId, transformedBook);
          }
        });
        ...
        console.warn('Individual search failed:', searchError.message);
        continue; // Continue with next search
      }
    }

    const finalResults = Array.from(allBooks.values()).slice(0, maxResults);

    console.log(`Found ${finalResults.length} unique books across multiple language searches`);
...
/**
 * Search for a book by ISBN (multiple languages)
...
const transformGoogleBookData = (googleBookItem) => {
  ...
  return {
    ...
    // Google Books returns `volumeInfo.authors` (lowercase). Normalize and keep short default "Unknown"
    author:
